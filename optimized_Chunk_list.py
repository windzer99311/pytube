...
def video_byte_range(video_url, num_segments):
    global video_chunk_list
    video_chunk_list = []  # Reset the list for each new download
...
def audio_byte_range(url, num_segments):
    global audio_chunk_list
    audio_chunk_list = []  # Reset the list for each new download
    if not os.path.exists(TEMP_FOLDER):
        os.makedirs(TEMP_FOLDER)
